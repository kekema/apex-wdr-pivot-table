...
create or replace package body "LIB4X_PIVOT_TABLE" as
  ...
  is
    ...
    l_filters_obj               json_object_t;	
    l_filter_obj                json_object_t;
    l_binds_obj                 json_object_t;	
    ...
    l_bind_seq                  pls_integer := 0;   
    
    function get_bind_expr(p_value varchar2) return varchar2
    is
      l_bind_name               varchar2(3);
    begin
      l_bind_seq := l_bind_seq + 1;
      l_bind_name := 'B' || l_bind_seq;  
      l_binds_obj.put(l_bind_name, p_value);
      return 'json_value(:' || p_filters_page_item ||', ''$.binds.' || l_bind_name || ''')';
    end get_bind_expr;	
	...
	is
      ...
      begin
		...
		l_condition_part := 'to_char(order_date, ''' || l_format || ''') = '|| get_bind_expr(l_value);
      ...
      begin
        for l_member_part_idx in 2 .. regexp_count(p_member, '\.') + 1 loop
      	  ...
      	  l_condition_part := l_condition_part || l_field || ' = '|| get_bind_expr(l_value);		  
        end loop;
        return l_condition_part;
      ...
      begin
        ...
        l_condition_part := 'to_char(order_date, ''YYYY'') = '|| get_bind_expr(l_value);
        return l_condition_part;
      ...
      begin
        l_value := substr(p_member, instr(p_member, '.', -1) + 1);
		l_condition_part := '''Q'' || to_char(order_date, ''Q'') = '|| get_bind_expr(l_value);
        return l_condition_part;
  ...
  begin
    ...
    if (l_filters is not null) then
	  l_filters_obj := json_object_t.parse(l_filters);
	  l_binds_obj := new json_object_t();
      l_filters_arr := l_filters_obj.get_Array('filters');
        ...
        if (p_excl_filters is null) or (instr(',' || replace(p_excl_filters, ' ') || ',', ',' || l_filter_name || ',') <= 0) then
          ...
          for l_members_arr_idx in 0 .. l_members.get_size - 1 loop
            ...
            if l_condition_part is null then
              ...
              l_condition_part := l_filter_name || ' = '|| get_bind_expr(l_value);    
            end if;
      ...
      l_filters_obj.put('binds', l_binds_obj);
      apex_session_state.set_value(p_filters_page_item, l_filters_obj.to_Clob);    	  
    end if;
    return l_condition;
  end get_condition;
